...
async def process_send_form_manager(message: Message):
    ...
    if message.from_user.id in user_dict:
        ...
        await bot.send_message(message.from_user.id, 'Спасибо!\n'
                                                     '\n'
                                                     'В начале следующего рабочего дня на указанную в заявке почту Вы получите письмо от платформы '
                                                     '<a href="https://brandmetrika.shopmetrics.com/">Shopmetrics</a> - для смены пароля.\n'
                                                     '\n'
                                                     'Следуйте инструкциям из письма для того чтобы установить пароль и зайти в свой Личный Кабинет 😊\n'
                                                     'Ваш логин для входа - это адрес Вашей эл.почты. - <b>до</b> знака "<b>@</b>".',
                         parse_mode='HTML',
                         reply_markup=faq_keyboard)
...
async def process_duration_payment(message: Message):
    await message.answer(text='Платежная ведомость формируется к 10му числу следующего месяца. '
                              ...
                              '\n'
                              'Пример: \n'
                              'Вы провели проверки 1, 10, 12 и 25 августа. Данные проверки будут включены в платёжную ведомость <b>10</b> сентября. '
                              'В этот же день Вы получите письмо на электронную почту с итоговой суммой оплаты по этим проверкам. '
                              'Во временной промежуток с <b>11</b> сентября до <b>20</b> октября Вам будут выставлены задачи в выбранной вами платежной системе и начислены денежные средства.',
                         parse_mode='HTML',
                         reply_markup=faq_keyboard)
...
async def process_help_command(message: Message):
    await message.answer(text='Привет ещё раз! 👋 \nЯ бот из BRANDMETRIKA и моя задача предоставить полную информацию по работе Тайным покупателем.\n'
                              'Выберите интересующий Вас вопрос из меню.',
                         reply_markup=faq_keyboard)
...
async def send_answer(message: Message):
    await message.answer(text='Извините, я Вас не понимаю 😵‍💫\n'
                              'Попробуйте отвечать кнопками на меню.',
                         reply_markup=faq_keyboard)
