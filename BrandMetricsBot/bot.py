...
async def process_fillform_command(message: Message, state: FSMContext):
    await message.answer(text='Сейчас мы поможем оформить короткую анкету для регистрации на портале Shopmetrics. Оформление займёт не более 1 минуты.\n'
                              '\n'
                              'Приступая к оформлению анкеты, вы даёте согласие на её обработку нашим персоналом для регистрации на портале Shopmetrics.\n'
                              '\n'
...
async def warning_not_city(message: Message):
    await message.answer(
        text='Введите, пожалуйста, населённый пункт проживания (без "г.", "гор.", "пос.")\n\n'
...
async def process_payment(message: Message):
    await message.answer(text='Гарантийное письмо, имеющее Юридическую силу запрашивается у Руководителя проекта – через Координатора проекта.',
                         reply_markup=faq_keyboard)
...
async def process_duration_payment(message: Message):
    await message.answer(text='Оплата для Физических Лиц производится через платёжный сервис <a href="https://solarstaff.com/">Solar Staff</a>, для Самозанятых – через сервис <a href="https://qugo.ru/">Qugo</a>.\n'
                              '\n'
                              'Платёжная ведомость за текущий месяц формируется <i>15 числа следующего месяца</i>. И, в течение 30 календарных дней, происходит выплата.\n'
                              '\n'
                              'Например, Вы провели проверку 25 августа. Данная проверка будет включена в платёжную ведомость 15 сентября. Во временной промежуток с 16 сентября до 15 октября будет произведена оплата.',
                         parse_mode='HTML',
                         reply_markup=faq_keyboard)
